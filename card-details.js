...
(() => {
  ...
  window.ChairCardDetails=Object.freeze({create({getData,commit,deletion,identity,escape:E,toast}){
    ...
    function save(message,change,after=()=>{}){
    ...
    dialog.addEventListener("submit",e=>{
      ...
      if(kind==="description")save("更新描述",t=>t.notes=v.notes,()=>{delete state.drafts.description;state.editing=null;});
      else if(kind==="title")save("修改卡片名稱",t=>t.title=v.title.trim(),()=>{delete state.drafts.title;state.editing=null;});
      else if(kind==="comment")save("新增留言",(t,c)=>c.comments.push({id,text:v.comment.trim(),author:identity.identity,createdAt:now}),()=>delete state.drafts.comment);
      else if(kind==="item")save("新增待辦項目",(t,c)=>{const l=c.checklists.find(l=>l.id===form.dataset.list);if(!l)throw Error("清單已變動。");l.items.push({id,text:v.text.trim(),done:false});},()=>delete state.drafts["item:"+form.dataset.list])&&dialog.querySelector(`[data-card-form="item"][data-list="${form.dataset.list}"] input`)?.focus();
      else if(kind==="label")save(p.kind==="edit-label"?"修改標籤":"新增標籤",(t,c)=>{if(!Object.hasOwn(M.colors,v.color))throw Error("請選擇標籤顏色。");const old=c.labels.find(l=>l.id===p.id);if(old)Object.assign(old,{name:v.name.trim(),color:v.color});else c.labels.push({id,name:v.name.trim(),color:v.color});},closePanel);
      else if(kind==="dates"){
        ...
        save("更新日期",(t,c)=>{t.date=v.date;c.startDate=v.startDate;c.dueTime=v.dueTime;},closePanel);
      }else if(kind==="member")save("指派卡片成員",(t,c,d)=>{
      ...
      else if(kind==="checklist")save("新增待辦清單",(t,c)=>c.checklists.push({id,title:v.text.trim(),items:[]}),closePanel);
      else if(kind==="rename-checklist")save("修改清單名稱",(t,c)=>c.checklists.find(l=>l.id===p.id).title=v.text.trim(),closePanel);
      else if(kind==="edit-item")save("修改待辦項目",(t,c)=>c.checklists.find(l=>l.id===p.list).items.find(i=>i.id===p.id).text=v.text.trim(),closePanel);
      else if(kind==="edit-comment")save("編輯留言",(t,c)=>{const m=c.comments.find(m=>m.id===p.id&&m.author===identity.identity);if(!m)throw Error("無法編輯這則留言。");m.text=v.text.trim();m.updatedAt=now;},closePanel);
      else if(kind==="attachment")save("新增附件連結",(t,c)=>{const url=M.safeURL(v.url);if(!url)throw Error("請填入有效的 http／https 連結，不含帳密。");c.attachments.push({id,kind:"link",url,name:v.name.trim()||new URL(url).hostname,createdAt:now});},closePanel);
      else if(kind==="move")save("移動卡片",(t,c,d)=>{const b=d.boards.find(b=>b.id===t.boardId),before=v.position==="top"?B.cards(d,b,v.listId).find(x=>x.id!==t.id)?.id||"":"";B.move(d,t.boardId,t.id,v.listId,before,identity.identity);},closePanel);
      else if(kind==="owner")save("修改負責職務",t=>{if(v.value&&!ChairBoards.units.some(u=>u.name===v.value))throw Error("請選擇職務。");t.owner=v.value;},closePanel);
      else if(kind==="status")save("修改工作狀態",(t,c,d)=>{if(!B.states.includes(v.value))throw Error("請選擇有效狀態。");setStatus(t,d,v.value);},closePanel);
      else if(kind==="copy")save("複製卡片",(t,c,d)=>{const next=structuredClone(t);next.id=id;delete next.sourceEntryId;next.title=v.text.trim();next.createdAt=now;next.updatedAt=now;next.card.comments=[];next.history=[{at:now,by:identity.identity,text:"由其他卡片複製",title:next.title,status:next.status,date:next.date}];next.boardOrder=d.tasks.filter(x=>x.boardId===t.boardId&&x.boardListId===t.boardListId).reduce((n,x)=>Math.max(n,x.boardOrder??0),-1)+1;d.tasks.push(next);},()=>{closePanel();toast("已複製到同一清單");});
    });
    dialog.addEventListener("click",e=>{
      ...
      if(action==="complete")save(current().status==="已完成"?"重新開啟工作":"完成工作",(t,c,d)=>{if(t.status==="已完成")setStatus(t,d,B.states.includes(c.previousStatus)&&c.previousStatus!=="已完成"?c.previousStatus:"未開始");else{c.previousStatus=t.status;setStatus(t,d,"已完成");}});
      if(action==="remove-label")save("移除標籤",(t,c)=>c.labels=c.labels.filter(l=>l.id!==id));
      if(action==="clear-dates")save("移除日期",(t,c)=>{t.date="";c.startDate="";c.dueTime="";},closePanel);
      if(action==="color-cover")save("設定封面",(t,c)=>c.cover={type:"color",value:el.dataset.color},closePanel);
      if(action==="clear-cover")save("移除封面",(t,c)=>c.cover=null,closePanel);
      if(action==="image-cover")save("更新圖片封面",(t,c)=>c.cover=c.cover?.id===id?null:{type:"attachment",id},closePanel);
      if(action==="remove-checklist"&&confirm("刪除這份待辦清單及其中項目？"))save("刪除待辦清單",(t,c)=>c.checklists=c.checklists.filter(l=>l.id!==id));
      if(action==="remove-item")save("移除待辦項目",(t,c)=>c.checklists.find(l=>l.id===list).items=c.checklists.find(l=>l.id===list).items.filter(i=>i.id!==id));
      if(action==="remove-comment"&&confirm("刪除這則留言？"))save("刪除留言",(t,c)=>c.comments=c.comments.filter(m=>m.id!==id||m.author!==identity.identity));
      if(action==="remove-attachment"&&confirm("從卡片移除此附件？"))save("移除附件",(t,c)=>{c.attachments=c.attachments.filter(a=>a.id!==id);if(c.cover?.id===id)c.cover=null;});
      if(action==="download")download(M.get(current()).attachments.find(a=>a.id===id));
    });
    dialog.addEventListener("change",e=>{
      if(!state)return;const el=e.target;
      if(el.id==="detailList")save("移動卡片",(t,c,d)=>B.move(d,t.boardId,t.id,el.value,"",identity.identity));
      if(el.dataset.checkItem){const checked=el.checked;save(checked?"完成待辦項目":"重新開啟待辦項目",(t,c)=>c.checklists.find(l=>l.id===el.dataset.list).items.find(i=>i.id===el.dataset.checkItem).done=checked)||render();}
      if(el.dataset.memberId){const checked=el.checked;save(checked?"指派卡片成員":"移除卡片成員",(t,c,d)=>{if(!checked)c.members=c.members.filter(m=>m.id!==el.dataset.memberId);else{const m=d.workUnits.find(u=>u.id===board().unitId)?.members.find(m=>m.id===el.dataset.memberId);if(!m)throw Error("成員名單已變動。");c.members.push({id:m.id,name:m.name});}})||render();}
    ...
    async function attachFile(file){
      ...
      try{const data=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onerror=()=>reject(Error("讀取附件失敗。"));reader.onload=()=>resolve(reader.result);reader.readAsDataURL(file);});
        ...
        save("新增檔案附件",(t,c)=>c.attachments.push({id:crypto.randomUUID(),kind:"file",name:file.name.slice(0,200),size:file.size,mime,data:encoded,createdAt:new Date().toISOString()}),closePanel);
