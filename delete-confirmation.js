...
(() => {
  window.ChairDeletion=Object.freeze({create({getData,commit}){
    ...
    $('confirmDeletion').onclick=async()=>{
      ...
      try{
        await commit(d=>ChairBoardModel.remove(d,request.kind,request.id,$('deleteNameInput').value),request.revision);
    ...
    return {open({kind,id,revision,after,label="卡片"}){
      ...
      $('deleteImpact').textContent=kind==='board'?'將刪除這張空看板及其清單設定。可由主席在雲端版本中復原。':(label==='工作'?'這筆工作若已放入看板，對應卡片也會同步刪除。':'這張卡片就是同一筆工作。')+'刪除後，工作清單與月曆中的這筆工作也會移除，描述、待辦、留言、附件及修改紀錄一併刪除，無法復原。';
