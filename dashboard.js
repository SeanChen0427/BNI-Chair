...
(async () => {
  ...
  try{data=window.ChairAuth?.enabled?await ChairCloud.init():S.read();}catch(e){try{data=S.read();}catch{}storageError(e);}
  async function commit(change,revision=data.revision){if(blocked)throw Error("資料讀取異常，已停止寫入。請到系統設定匯出備份。");if(window.ChairCloud?.busy)throw Error("正在儲存，請稍候。");const wait=$("cloudSaving");wait?.showModal();try{const next=await S.save(change,revision);data=next;render();}finally{wait?.close();}}
  ...
  function renderPower(){
    ...
    const card=({meeting:m,power:p},label)=>`<article class="power-card"><div class="power-meta"><span>${label}</span><time>${E(ChairPowerOfOne.period(p))}</time></div><h3>${E(p.title)}</h3><p class="power-action">${E(p.action)}</p><a class="power-source" data-power-meeting="${E(m.id)}" href="${url(m)}">${E(m.title)} · 查看設定與會議 ↗</a></article>`;
    const past=g.past.map(({meeting:m,power:p})=>`<a class="power-past" data-power-meeting="${E(m.id)}" href="${url(m)}"><strong>${E(p.title)}</strong><span>${E(ChairPowerOfOne.period(p))} ↗</span></a>`).join('');
  ...
  function render(){
    ...
    $("overviewPeriod").textContent=`${today.slice(0,4)} 年 ${Number(today.slice(5,7))} 月 · 雲端共用`;
  ...
  const meetingWorkspace=ChairMeetingWorkspace.create({getData:()=>data,commit,identity,escape:E,toast,openTask:id=>openEditor("task",id),onCreateMeeting:prefill=>openMeeting("",prefill),onSourceMeeting:(id,sectionId)=>openMeeting(id,{sectionId}),onBack:()=>navigate(meetingReturn.hash),onSaved:id=>{if(parseMeetingRoute(activeHash)?.id==="new"){history.replaceState(null,"","#meetings/"+encodeURIComponent(id)+(["power","strategy"].includes(parseMeetingRoute(activeHash)?.params.get("view"))?"?view="+parseMeetingRoute(activeHash).params.get("view"):""));activeHash=location.hash;}}});
  ...
  $("messageForm").onsubmit=async e=>{e.preventDefault();const text=$("messageInput").value.trim();if(!text)return;try{await commit(d=>d.messages.push({id:crypto.randomUUID(),text,author:identity.identity,createdAt:new Date().toISOString()}));$("messageForm").reset();$("messageLength").textContent="0／1000";toast("留言已儲存至雲端");}catch(err){toast(err.message);}};
  ...
  $("editorForm").onsubmit=async e=>{e.preventDefault();if(!editorState)return;const values=Object.fromEntries(new FormData(e.currentTarget));values.title=values.title.trim();if(!values.title){$("editorError").textContent="請輸入名稱。";return;}
  ...
  $("readAll").onclick=async()=>{try{const keys=notices().map(n=>n.key);await commit(d=>d.read=[...new Set([...d.read,...keys])]);}catch(e){toast(e.message);}};
  const help={units:["選擇工作單位，再建立一張張有封面的看板。","看板內可原位新增卡片、自訂清單；拖曳卡片改變順序或移到其他清單。清單標示的狀態會套用到移入的工作。","點卡片可編輯原工作案件、職務與期限，月曆共用同一筆資料。卡片移動選單可供手機及鍵盤操作。","成員是本機協作名單，新增姓名不會發出邀請或開通權限。"],home:["按住區塊標題或 ⠿ 即可拖移，手機長按啟動，放開保存；系統設定可恢復預設排列。鍵盤可按空白鍵選取、上下键移動、Enter 保存、Escape 取消。","首頁本屆 KPI 可直接設定九項目標；留空顯示 —，修改後可查保存版本，目前尚未接入實績。","在會議內設定 Power of One，勾選在首頁顯示並儲存。首頁依期間呈現本期、即將開始及過往設定。","留言板可留下公告、提醒或交接內容，儲存後供主席與核心共用。","會員狀態讀取委員會已發布分析；會員名單可展開查閱，並顯示來源期間。","工作期限與會議日期會出現在月曆。點案件可繼續更新。"],tasks:["按新增工作，填名稱後可保存；負責職務與期限可以稍後補。","用待處理、我的工作、需協助或已完成切換清單。","修改案件後按儲存，月曆與首頁同步更新，修改紀錄保留。"],meetings:["新增會議先選種類，再進入議程；要更換時點「更換種類」。","選會前會或會後會，切換執掌後在各議程獨立填寫；議程可增減、改名。","每筆紀錄旁可加標籤、執行日期／例會場次及協作單位；選單位即列入工作，儲存後同步各執掌工作區。","工作追蹤可更新同一筆工作的狀態；會議結束與工作完成分開。雲端保存；衝突時保留輸入，核對後再儲存。"],calendar:["點日期看當日工作期限、會議、培訓及例會安排；點培訓可看詳細資訊。","從日期下方新增工作或會議，日期會先帶入；保存後才生效。","週二例會與國定假日／補假休會沿既有任期設定，例會時間未定。"],members:["此區讓核心幹部掌握分會整體紅綠燈人數與比例。","顯示目前在籍會員及最近已結束月份的已發布分析；缺少對應資料顯示 —，不當作零。"],resources:["保存常用文件或網站的連結。","只接受 http／https 網址，外部網站會在新分頁開啟。"],updates:["這裡列出新版各階段已完成及尚未接上的內容。"],settings:["匯出備份會下載此新版的本機內容，包含留言、工作、會議與資源。","登出會清除此分頁的登入；已保存在瀏覽器的工作內容仍保留。"]};
  ...
  window.addEventListener("storage",e=>{if(!window.ChairCloud?.ready&&e.key===S.KEY){try{data=S.read();render();boards.storageChanged();cardDetails.storageChanged();meetingWorkspace.storageChanged();termKpi.storageChanged();if(editorState)$("editorError").textContent="另一分頁更新了資料。目前輸入已保留，儲存前請重新核對。";}catch(err){storageError(err);}}});
  ...
  $("cloudHistory").onclick=async()=>{try{
    ...
    dialog.innerHTML='<h2>雲端保存版本</h2><p>復原會新增一個版本，保留復原前內容；僅主席可執行。</p><select aria-label="選擇雲端版本">'+list.map(v=>`<option value="${v.revision}">版本 ${v.revision} · ${E(v.identity)} · ${E(stamp(v.created_at))}</option>`).join('')+'</select><button data-preview>查看差異</button><pre class="cloud-diff"></pre><button data-restore disabled>復原此版本</button><button data-close>關閉</button>';
    let target=null,expected=null;dialog.querySelector('[data-close]').onclick=()=>dialog.close();
    dialog.querySelector('select').onchange=()=>{target=null;dialog.querySelector('[data-restore]').disabled=true;};
    dialog.querySelector('[data-preview]').onclick=async()=>{try{target=await ChairCloud.version(Number(dialog.querySelector('select').value));expected=data.revision;const changes=ChairCloud.diff(data,await ChairCloud.decode(target));dialog.querySelector('pre').textContent=changes.length?changes.map(c=>`${c.kind} · ${c.value?.title||c.id} · ${c.value===null?'移除':data[c.kind]?.some(x=>x.id===c.id)?'還原內容':'加入'}`).join('\n'):'與目前內容相同';dialog.querySelector('[data-restore]').disabled=identity.role!=='chair'||!changes.length;}catch(e){toast(e.message);}};
    dialog.querySelector('[data-restore]').onclick=async()=>{if(dirty()){toast('請先保存或取消編輯中的內容。');return;}if(!target||!confirm('將主席系統資料復原到版本 '+target.revision+'？目前版本 '+expected+' 仍會保留，可再復原。'))return;try{data=await ChairCloud.restore(target,expected);render();dialog.close();toast('已復原並儲存至雲端');}catch(e){toast(e.message);}};
    dialog.showModal();
  }catch(e){toast(e.message);}};
  if(window.ChairCloud?.ready)setInterval(async()=>{if(document.hidden||dirty()||editorState||document.querySelector('dialog[open]'))return;try{const next=await ChairCloud.poll();if(next){data=next;render();if(route==='meeting')meetingWorkspace.refresh();}}catch(e){ChairCloud.status('雲端連線暫停，儲存時會重新確認。',true);}},15000);
